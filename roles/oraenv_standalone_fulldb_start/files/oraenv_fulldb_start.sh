...
# process aruments
#
#dblist=`ps -ef|grep -i pmon|grep -v grep|cut -d"_" -f3`
dblist=`cat /oracle/stage/listnamefile_donotdelete|awk '{print $1}'`
for db in $dblist
do
NEWSID="$db"
echo "NEWSID = $NEWSID"
list=`cat /oracle/stage/listnamefile_donotdelete|grep -i $db|awk '{print $2}'`
SILENT='';
if [ $# -gt 0 ]; then
 for arg in $@
 do
...
spool /tmp/$NEWSID-startstatus-$hname.txt
