...
agstatus=`hdbsql -U hcuser2 -j -x -C "select UFLAG from $SCHEMA.usr02 where BNAME='IBMMON_AGENT' and MANDT='000'"|tail -1`
echo $sid > /tmp/itmuserstatus_hanadb_$sid
echo $agstatus >> /tmp/itmuserstatus_hanadb_$sid