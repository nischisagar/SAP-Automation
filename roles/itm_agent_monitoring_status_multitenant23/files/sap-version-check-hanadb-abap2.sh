...
dbsize=`hdbsql -U hcuser2  -j -x -C "SELECT ROUND(TO_DOUBLE(sum(disk_size)/1024/1024/1024),2) "DB_SIZE" FROM m_table_persistence_statistics" | tail -1`
...
echo "$dbsize" >> /tmp/$sidadm-sap-version-check-hanadb-abap2.log